...
//textroom stuffs	
var myroom_text = 9876;  // Text room I THINK
var janus_text = null;
var textroom = null;
var opaqueId_text = "textroomtest-"+Janus.randomString(12);
var participants = {}
var transactions = {}
							...
							janus.attach(
								{
									...
									opaqueId: opaqueId_text,
									success: function(pluginHandle) {
										$('#details').remove();
										textroom = pluginHandle;
										Janus.log("Plugin attached! (" + textroom.getPlugin() + ", id=" + textroom.getId() + ")");
										// Setup the DataChannel and create new textroom 1235
										var body = { 
											request: "setup" 
										};
...
function registerUsername() {
	if($('#username').length === 0) {
		...
		transactions[transaction] = function(response) {
			if(response["textroom"] === "error") {
				...
				if(response["error_code"] === 417) {
					// This is a "no such room" error: give a more meaningful description
					bootbox.alert(
						"<p>Apparently room <code>" + myroom_text + "</code> (the one this demo uses as a test room) " +
		...
		textroom.data({
			text: JSON.stringify(register_text),
